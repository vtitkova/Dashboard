package com.dmma.base.gwt.client.ui.estateInfo;


import com.dmma.base.gwt.shared.interfaces.PlaceInfoBig;
import com.google.gwt.core.client.GWT;
import com.google.gwt.safehtml.shared.SafeHtmlBuilder;
import com.google.gwt.user.client.ui.HTML;
import com.google.gwt.user.client.ui.PopupPanel;
import com.google.gwt.user.client.ui.UIObject;

public class PlaceInfoBigPopup{
	private PopupPanel popupPanel;

	public PlaceInfoBigPopup(PlaceInfoBig info) {
		popupPanel = new PopupPanel();
		popupPanel.setAutoHideEnabled(true);
		if (!GWT.isScript())
			popupPanel.setTitle(this.getClass().getName());
		SafeHtmlBuilder sb = new SafeHtmlBuilder();
		new PlaceCellBig().render(null, info, sb);
		popupPanel.add(new HTML(sb.toSafeHtml()));
		
	}

	public void showRelativeTo(UIObject target){
		popupPanel.showRelativeTo(target);
	}

	public static void show(PlaceInfoBig info, UIObject target){
		PlaceInfoBigPopup instance = new PlaceInfoBigPopup(info);
		instance.showRelativeTo(target);
	}

}
