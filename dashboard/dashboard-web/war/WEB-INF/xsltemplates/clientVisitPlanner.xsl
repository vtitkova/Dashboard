...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="clientVisitPlan">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			...
			<fo:page-sequence master-reference="signing-master"
				...
				<!-- static header with LOGO and text used on first page -->
									...
									<fo:table-row>
										...
										<fo:table-cell font-size="10pt" padding="0"
											...
											<fo:block text-align="right">
												<fo:external-graphic src="{$EM1.LOGO}" />
												<fo:inline>
												</fo:inline>
											</fo:block>
										</fo:table-cell>
				...
				<!-- static footer used on first page -->
				...
				<fo:flow flow-name="xsl-region-body">
					<xsl:apply-templates select="clientVisitPlanHeader" />
					<xsl:apply-templates select="clientVisits" />
				</fo:flow>
			</fo:page-sequence>
		</fo:root>
	</xsl:template>
	
	<xsl:include href="sub/clientVisitPlanHeaderPart.xsl"/> 
	<xsl:include href="sub/clientVisitsPart.xsl"/> 
