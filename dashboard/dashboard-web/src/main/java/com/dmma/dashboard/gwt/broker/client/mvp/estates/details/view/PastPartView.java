...
public class PastPartView extends BaseComposite{
	...
	public PastPartView(EstateViewingDTO estateViewing, Integer estateId) {
		...
		dpHeaderPanel = new FlowPanel();
		dpHeaderPanel.add(new HTML(dpHeader));
		dp = new DisclosurePanel(dpHeader);
	...
	public void addOneRow(ClientVisitDTOS cv){
		...
		clientVisitTable.setHTML(  row, 3, ClientVisitStatusType.createIconHTML(cv.getStatus()));
		
		if(ClientVisitStatusType.isPlaningToGo.getId().equals(cv.getStatus())){
			displayWarning();
		} 
	}
	...
	private void displayWarning() {
		if(!displayWarning){
			displayWarning = true;
			dp.getHeader().addStyleName("pastPartViewWarning");
		}
	}
	...
	public void setClientVisits(ArrayList<ClientVisitDTOS> clientVisits){
		...
		clientVisitTable.setHTML(0, 3, DashboardMessages.MSG.status());
