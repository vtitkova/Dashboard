package com.dmma.dashboard.gwt.core.client.mvp.tip.tipform.view;
...
public class AbstractTipFormView extends Composite implements AbstractTipFormPresenterDisplay{
	...
	public AbstractTipFormView(int mode) {
	...
	protected void createComponentsAndTitles() {
		// ID
		deatilsTable.setHTML(   0, 0, DashboardMessages.MSG.id());
		idLabel = new Label();
		...
		//MEGLER
		deatilsTable.setHTML( meglerPos, 0, DashboardMessages.MSG.broker());
		brokerLA = new Label();
		...
		//DIRECTION
		deatilsTable.setHTML( 2, 0, DashboardMessages.MSG.banker());
		...
		//BANKER
		deatilsTable.setHTML( bankerPos, 0, DashboardMessages.MSG.banker());
		bankerLA = new Label();
		...
		//ESTATE
		deatilsTable.setHTML( 5, 0, DashboardMessages.MSG.estate());
		estateLA = new Label();
		...
		//COMMENTS
		deatilsTable.setHTML(7, 0, DashboardMessages.MSG.comments());
		comments = new TextArea();
		deatilsTable.setWidget(7, 1, comments);
	
		//CREATED
		deatilsTable.setHTML(8, 0, BaseMessages.MSG.created());
		created = new DateBox();
		created.setFormat(new DateBox.DefaultFormat(BaseFormats.DATE_FORMAT));
		deatilsTable.setWidget(8, 1, created);
		
		//STATUS
		deatilsTable.setHTML(9, 0, DashboardMessages.MSG.status());
		tipStatusTypeLB  = new ListBox();
		BaseListBoxUtils.setItemsNAToLB(tipStatusTypeLB);
		tipStatusTypeLB.setWidth(LB_WIDTH);
		deatilsTable.setWidget(9, 1, tipStatusTypeLB);
		
		//REPLY PART
		deatilsTable.setHTML(10, 0, DashboardMessages.MSG.modificationComments());
		statusComments = new TextArea();
		deatilsTable.setWidget(10, 1, statusComments);
		


		deatilsTable.setHTML(11, 0, DashboardMessages.MSG.modificationDate());
	...
	protected void renderTipDirectionImageAndLabel() {
		...
		if(is_I_send)
			tipDirectionTypeImage.setResource(DashboardImages.IMG.arrowDownGrey22());
		else 
	...
	protected void renderTipStatusTypeLBBasedOnMode(){
		...
		if(is_I_send){
			...
			BaseListBoxUtils.setSelectedItemByValue(tipStatusTypeLB, ""+type.getId());
